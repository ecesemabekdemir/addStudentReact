...
function App() {
  ...
  function showModal() {   // butona onclick vericez modal açılması içn
    setIsModalOpen(true);
   
  }

  function closeModal() { // modal kapanması için
  ...
  function addStudentRecord(record) {   // yeni ekleme record parametresi formdan aldığım bilgi
  ...
  return (
    <div className="container">
      <h1>
        Öğrenci bilgi sistemi <button className="addBtn" onClick={showModal}>Yeni Ekle</button> 
      </h1>
     ...
     {data.map((x) => (
      ...
      <AddStudent isOpen={isModalOpen}  closeModal={closeModal} addStudentRecord={addStudentRecord} />
    </div>
  );
}
  ...
  return (
    <dialog id="modal" open={isOpen}>
      ...
      <form className="formModal" method="dialog" autoComplete="off" onSubmit={handleSubmit}>
        ...
        <input required type="email" name="ePosta" placeholder="ePosta giriniz" />
        ...
        <button className="exit" type="button"
          onClick={closeModal}> Vazgeç</button>
      </form>
    </dialog>
  );
}
